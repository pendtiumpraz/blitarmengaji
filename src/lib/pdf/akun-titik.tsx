...
const styles = StyleSheet.create({
  ...
  // Lebar kolom: No / Titik / Kecamatan / Email / Password
...
function AkunTitikDocument({ rows, generatedAt }: { rows: TitikRow[]; generatedAt: Date }) {
  return (
    <Document
      title="Daftar Akun Pengelola Titik — Blitar Mengaji"
      author="Blitar Mengaji"
      subject="Daftar Akun Pengelola Titik"
    >
      <Page size="A4" style={styles.page}>
          ...
          <View style={styles.brandRow}>
            ...
            <View>
              <Text style={styles.brandTitle}>
                BLITAR MENGAJI — Daftar Akun Pengelola Titik
              </Text>
              <Text style={styles.brandSub}>
                Dokumen rahasia · kredensial akses untuk pengelola titik ngaji
              </Text>
            </View>
        ...
        {/* TABEL */}
        <Text style={styles.sectionTitle}>Daftar Akun ({rows.length} titik)</Text>
        <View style={styles.table}>
          <View style={styles.thead} fixed>
            <Text style={[styles.th, styles.cNo]}>No</Text>
            <Text style={[styles.th, styles.cTitik]}>Titik</Text>
            <Text style={[styles.th, styles.cKec]}>Kecamatan</Text>
            <Text style={[styles.th, styles.cEmail]}>Email</Text>
            <Text style={[styles.th, styles.cPass]}>Password</Text>
          </View>

          {rows.length === 0 ? (
            <Text style={styles.empty}>Belum ada data akun pengelola titik.</Text>
          ) : (
            rows.map((r, i) => (
              <View
                key={`${r.email}-${i}`}
              ...
              >
                ...
                <Text style={[styles.td, styles.cTitik]}>{r.titik}</Text>
                <Text style={[styles.td, styles.cKec]}>{r.kecamatan}</Text>
...
/** Render daftar akun pengelola titik menjadi PDF buffer (Uint8Array) untuk dikirim sebagai Response. */
export async function renderAkunTitikPdf(rows: TitikRow[]): Promise<Uint8Array<ArrayBuffer>> {
  const buf = await renderToBuffer(<AkunTitikDocument rows={rows} generatedAt={new Date()} />);
